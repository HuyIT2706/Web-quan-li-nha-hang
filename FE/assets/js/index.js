...
const fetchProducts = async () => {
    try {
        const res = await fetch('http://localhost/quanlinhahang/BE/api_product.php');
...
const fetchProductById = async id => {
    const res = await fetch(`http://localhost/quanlinhahang/BE/api_product.php?id=${id}`);
...
const renderPagination = () => {
    ...
    paginationContainer.innerHTML = Array.from({ length: totalPages }, (_, i) => i + 1)
        .map(i => `<a href="#menu-section" class="action_page--item${i === currentPage ? ' active' : ''}" onclick="goToPage(${i})">${i}</a>`)
        .join('');
};
...
async function fetchAndRenderTables() {
    try {
        const res = await fetch('http://localhost/quanlinhahang/BE/tables.php');
...
async function checkUserAndTable() {
    const res = await fetch('http://localhost/quanlinhahang/BE/checkin.php', {
        method: 'GET',
...
async function updateTableStatus(tableId, status, tableName, actionWord) {
    try {
        ...
        if (oldTableId && oldTableId !== tableId.toString()) {
            const resetOldTable = await fetch('http://localhost/quanlinhahang/BE/update_table.php', {
                method: 'POST',
        ...
        // Cập nhật bàn mới
        const res = await fetch('http://localhost/quanlinhahang/BE/update_table.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                table_id: tableId,
        ...
        const res2 = await fetch('http://localhost/quanlinhahang/BE/select_table.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
...
const handdlAddCard = async (items) => {
    ...
    try {
        ...
        if (existingItem) {
            ...
            const response = await fetch('http://localhost/quanlinhahang/BE/api_cart_update.php', {
                method: 'POST',
            ...
            const response = await fetch('http://localhost/quanlinhahang/BE/api_order.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
                body: JSON.stringify({
...
async function fetchCart() {
    const res = await fetch('http://localhost/quanlinhahang/BE/api_cart.php', {
        credentials: 'include'
    });
    return await res.json();
}
...
document.querySelector('.cart-dark-table').addEventListener('click', async function (e) {
    ...
    if (btn) {
        ...
        await fetch('http://localhost/quanlinhahang/BE/api_cart_remove.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            credentials: 'include',
...
document.querySelector('.cart-dark-table').addEventListener('click', async function (e) {
    ...
    if (btn) {
        ...
        // Gửi API cập nhật
        const res = await fetch('http://localhost/quanlinhahang/BE/api_cart_update.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            credentials: 'include',
...
async function fetchAllPendingOrders() {
    const res = await fetch('http://localhost/quanlinhahang/BE/api_order_detail.php', {
        credentials: 'include'
    });
    return await res.json();
}
...
const logoutAcount = async () => {
    try {
        const response = await fetch('http://localhost/quanlinhahang/BE/logout.php', {
            method: 'POST',
            credentials: 'include'
        });
        const data = await response.json();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', () => {
            ...
            suggestionBox.innerHTML = matched.slice(0, 5)
                .map(p => `<li>${p.name}</li>`)
                .join('');
            ...
            suggestionBox.querySelectorAll('li').forEach(li => {
                li.addEventListener('click', () => {
                    ...
                    const filtered = allProducts.filter(p =>
                        p.name.toLowerCase().includes(selectedText.toLowerCase())
                    );
        ...
        document.addEventListener('click', (e) => {
            if (!e.target.closest('.menu-filter__search-row')) {
                suggestionBox.innerHTML = '';
                suggestionBox.style.display = 'none';
            }
        });
...
const renderFilteredProducts = (products) => {
    ...
    productContainer.innerHTML = products.length
        ...
        : '<p style="color: white;">Không tìm thấy sản phẩm phù hợp.</p>';

    const pagination = document.querySelector('.list--action_page');
    if (pagination) {
        // Ẩn phân trang nếu đang lọc hoặc không có sản phẩm
        pagination.style.display = (products.length < allProducts.length) ? 'none' : 'flex';
    }
};
