...
document.addEventListener('DOMContentLoaded', function() {
  // Tìm nút "Đơn hàng đã mua" trong dropdown
  const btnDonHang = document.querySelector('.account-dropdown__item a .fa-shopping-bag')?.parentElement;
  if (btnDonHang) {
    btnDonHang.addEventListener('click', function(e) {
      e.preventDefault();
      // Hiện khu vực đơn hàng, ẩn các khu vực khác nếu cần
      const ordersDiv = document.getElementById('orders');
      if (ordersDiv) {
        ordersDiv.scrollIntoView({ behavior: 'smooth' });
      }
      // Nếu muốn show/hide các khu vực khác, thêm code tại đây
    });
  }
});
