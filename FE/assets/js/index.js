...
if (slider) {
  ...
  console.warn("Không tìm thấy phần tử slider!");
}
...
function renderOrders(orders) {
  if (!orders.length) {
    document.getElementById('orders').innerHTML = '<p>Không có đơn hàng nào.</p>';
    return;
  }
    ...
    html += `
      <div class="order-card">
        <div class="order-card__header">
          ...
          <span class="order-card__table">Bàn ${order.table_number}</span>
        </div>
        ${order.items.map(item => `
          <div class="order-card__item">
            ...
            <div class="order-card__info">
              ...
              <div class="order-card__note">${item.note ? item.note : 'Không có ghi chú'}</div>
              <div class="order-card__qty">x${item.quantity}</div>
            </div>
            <div class="order-card__price">${Number(item.price).toLocaleString()} đ</div>
          </div>
        `).join('')}
...
// Chỉ chạy khi có #orders trên trang
