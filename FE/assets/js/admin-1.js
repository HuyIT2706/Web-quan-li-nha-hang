...
// Hiển thị chi tiết các đơn hàng của một sản phẩm
...
async function detailOrderProduct(arr, idProduct) {
    const res = await fetch(`../BE/api_product_orders.php?product_id=${idProduct}`);
    const data = await res.json();
...
document.querySelectorAll('.modal.detail-order-product .modal-close').forEach(btn => {
...
function showThongKe(arr, mode) {
    ...
    for (let i = 0; i < mergeObj.length; i++) {
        ...
        <tr>
            ...
            <td>
                <button class="btn-detail product-order-detail" data-id="${mergeObj[i].id}">
    ...
    // Gán lại sự kiện click cho các nút "Chi tiết" sau khi render bảng
    document.querySelectorAll(".product-order-detail").forEach(item => {
        let idProduct = item.getAttribute("data-id");
        item.onclick = () => {
            detailOrderProduct(null, idProduct);
        };
    });
}

      async function thongKe(mode = 0) {
    const keyword = document.getElementById('form-search-tk').value.trim();
    const category = document.getElementById('the-loai-tk').value.trim();

    try {
        const res = await fetch(`../BE/api_statistic.php?category=${encodeURIComponent(category)}&keyword=${encodeURIComponent(keyword)}`);
        const data = await res.json();
        ...
        const formattedData = data.map(item => ({
            ...
            doanhthu: parseInt(item.revenue)
        }));

        showThongKe(formattedData, mode);
    } catch (err) {
